...
+ **逆向 IDFT（逆離散傅立葉變換）** 公式：
  ...
  + 計算每個 $n$ 的值：  
    ...
    + $b[2] = \frac{1}{\sqrt{4}} \left( 5 \cdot e^{i \cdot 0} + (-1+i) \cdot e^{i \cdot \pi} + (-1) \cdot e^{i \cdot 2 \pi} + (-1-i) \cdot e^{i \cdot 3 \pi} \right) = \frac{1}{2} \left( 5 + (-1+i) (-1) + (-1) (1) + (-1-i) (-1) \right) = \frac{1}{4} \left( 5 + (1-1i) + (-1) + (1+1i) \right) = \frac{6}{2} = 3$  

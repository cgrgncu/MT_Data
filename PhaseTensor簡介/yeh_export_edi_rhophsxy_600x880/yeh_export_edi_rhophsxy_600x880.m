%**************************************************************************
%   Name: yeh_export_edi_rhophsxy_600x880.m 
%   Copyright:  
%   Author: HsiupoYeh 
%   Version: v20221003a
%   Description: 輸出HsiupoYeh建議的大地電磁法視電阻率及相位圖(WxH=600x880)
%                經過測試可用於以下MATLAB版本:「R2009a」、「R2014a」、
%               「R2014b」。
%**************************************************************************
function ExportEdiPNG=yeh_export_edi_rhophsxy_600x880(Input_WinGLinkEdi_mat,Input_title,output_png_path,output_png_file_name)
%----------------------------------------------------------------------
    ...
    if ~exist('Input_WinGLinkEdi_mat','var') || ...
        ...
        disp('output_png_file_name=[''yeh_export_edi_rhophsxy_600x880_v20221003a.png''];')
        disp('ExportEdiPNG=yeh_export_edi_rhophsxy_600x880(Input_WinGLinkEdi_mat,Input_title,output_png_path,output_png_file_name);')
    ...
    % 狀況下(且螢幕沒有旋轉，寬度=1280，高度=720)運行此程式，以免錯誤發生。
        %--
        % 使用的figure尺寸暫定為500x500，也就是每個小部分圖片最大不超過600x600。
        % 這樣讓開發期間比較美觀的Debug。

%**************************************************************************
%   Name: yeh_export_edi_rhophs_C_300x480.m 
%   Copyright:  
%   Author: HsiupoYeh 
%   Version: v20221009a
...
%               「R2014b」。
%**************************************************************************
function ExportEdiPNG=yeh_export_edi_rhophs_C_300x480(varargin)
    %======================================================================
    %----------------------------------------------------------------------
    % 輸出預設值
    ...
    if nargin==4 
        ...
        disp('output_png_file_name=[''yeh_export_edi_rhophs_C_300x480_v20221009a.png''];')
    ...
    % 。但強制要求主機必須有硬體顯示卡存在，故虛擬機中或顯示卡驅動異常的狀況下無
    % 法使用進階功能，以致於繪圖不太美觀。
    ...
    %----------------------------------------------------------------------
    % MATLAB產生的繪圖視窗figure只能比當前螢幕尺寸小，目前只允許螢幕解析度大於
    % 720p的狀況下(且螢幕沒有旋轉，寬度=1280，高度=720)運行此程式，以免錯誤發生。
    %--
...
my_ylim_vector=[-2,5];
    ...
    set(axes_handle,'Position',[100,100,main_axes_width,figure_max_height])
    %--
    % 擷取視窗像素資料
...
errorbar(log10(Input_WinGLinkEdi_mat.FREQ.Data),log10(Input_WinGLinkEdi_mat.RHOXX.Data),Input_WinGLinkEdi_mat.RHOXX_ERR.Data,'.','color',[0,0.5,0]);
errorbar(log10(Input_WinGLinkEdi_mat.FREQ.Data),log10(Input_WinGLinkEdi_mat.RHOYY.Data),Input_WinGLinkEdi_mat.RHOYY_ERR.Data,'.','color',[140/255,60/255,140/255]);
    %--------------------------------------------------------------------------
    %把標題設定回去
...
temp_ylabel_xyz(1)=-18;
    ...
    %----------------------------------------------------------------------
    % 設定圖說
    legend_handle=legend('XY','YX','XX','YY','Location','SouthOutside');
    ...
    if verLessThan('matlab', '8.4.0.150421')
        disp('提示:正在使用舊的繪圖引擎...')
set(legend_handle,'PlotBoxAspectRatio',[100,5,1])
set(legend_handle,'Position',[axes_left_pexel_index+6,4,200,20]);
    else
        disp('提示:正在使用新的繪圖引擎...')
        opengl_information=opengl('data');
        if opengl_information.Software==1
        ...
        legend_handle.EntryContainer.Children(3).Label.FontSmoothing='off';
        legend_handle.EntryContainer.Children(4).Label.FontSmoothing='off';
legend_handle.ItemTokenSize=[20,18];
set(legend_handle,'Position',[axes_left_pexel_index+8,4,200,20]);        
    end
    %----------------------------------------------------------------------
    % 截圖
    temp_frame=getframe(figure_handle);
...
my_ylim_vector=[-180,180];
    ...
    % 調整自訂Position，使座標軸依照所需尺寸正確地出現在畫面中。
    set(axes_handle,'Units','pixels')
set(axes_handle,'Position',[100,100,main_axes_width,165])% 165=main_axes_width*0.75加微調    
    %--
    % 擷取視窗像素資料
    temp_frame=getframe(figure_handle);
    %--
...
errorbar(log10(Input_WinGLinkEdi_mat.FREQ.Data),(Input_WinGLinkEdi_mat.PHSXX.Data),Input_WinGLinkEdi_mat.PHSXX_ERR.Data,'.','color',[0,0.5,0]);
errorbar(log10(Input_WinGLinkEdi_mat.FREQ.Data),(Input_WinGLinkEdi_mat.PHSYY.Data),Input_WinGLinkEdi_mat.PHSYY_ERR.Data,'.','color',[140/255,60/255,140/255]);
%--------------------------------------------------------------------------
% 設定XTick
set(axes_handle,'Xlim',my_xlim_vector)
set(axes_handle,'XTick',[my_xlim_vector(1):my_xlim_vector(2)])
